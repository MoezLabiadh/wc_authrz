...
for loc in locs:
    print ('working on {} '.format(str(loc)))
    df_loc = df.loc[df['Geographic_Region-ID'].astype(str) == str(loc)] 
    
    df_loc = df_loc.rename(columns={'Quota_Requested_MT': 'Quota Requested', 
                                  'Total_Quota_Approved': 'Quota Approved'})
        

    df_gr = df_loc.groupby(['Year', 'Species_Group']).agg({'Quota Requested':'sum',
                                                           'Quota Approved':'sum',
    ...
    sp_grps = sorted(list(df_loc['Species_Group'].unique()))
    ...
    t_txt = 'Wild Aquatic Plant Harvest - {}'.format (str(loc))  
    ...
    for i, sp_g in enumerate(sp_grps):
        ...
        for ind in range (0,filt_hr.shape[0]):
            miss = df_loc.loc[(df_loc['Year'] == filt_hr['Year'].iloc[ind]) & 
                              (df_loc['Species_Group'] == sp_g)]['harv_is_missing'].iloc[0]
    ...
    filename = 'graph_geoloc_{}.png'.format (str(loc))
    fig.savefig(os.path.join(wks, 'outputs', 'plots', 'by_geoloc', filename), dpi=150)
