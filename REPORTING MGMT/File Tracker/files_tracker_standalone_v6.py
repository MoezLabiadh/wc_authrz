...
def create_rpt_01(df_tnt,df_ats):
    ...
    for df in [df_01,df_01_nw,df_01_rp]:
        ...
        df['mtr01']  = np.where((today - df['Received Date']).dt.days.isin([0, np.nan]), 
                                np.nan, 
                                ((today - df['Received Date']).dt.days) - df['Total On Hold Time'])
        
        df.loc[df['mtr01'] < 0, 'mtr01'] = (today - df['Received Date']).dt.days
...
def create_rpt_02(df_tnt,df_ats):
    ...
    for df in [df_02,df_02_nw,df_02_rp]:
        ...
        df['mtr02'] = np.where((df['Submission Review Complete Date'] - df['Received Date']).dt.days.isin([0, np.nan]), 
                                np.nan, 
                                ((df['Submission Review Complete Date'] - df['Received Date']).dt.days) - df['Total On Hold Time'])
        
        df.loc[df['mtr02'] < 0, 'mtr02'] = (df['Submission Review Complete Date'] - df['Received Date']).dt.days
...
def create_rpt_08 (df_tnt,df_ats):
    ...
    for df in [df_08,df_08_nw,df_08_rp]:
        ...
        df['mtr17'] = np.where((df['COMPLETED DATE'] - df['RECEIVED DATE']).dt.days.isin([0, np.nan]), 
                        np.nan, 
                        ((df['COMPLETED DATE'] - df['RECEIVED DATE']).dt.days) - df['Total On Hold Time'])
