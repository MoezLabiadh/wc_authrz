...
def create_rpt_03_1 (today,df03):
    ...
    df_031= df_03.loc[(df_03['First Nation Start Date'].notnull()) &
                        (df_03['First Nation Completion Date'].isnull())]
...
def add_readme_page(readme_xlsx, filename):
 ...
 def main():
  ...
  # The last day of previous month. Will be used to calculate Metrics
  today = date.today()
  ...
  ats_oh_f = os.path.join(wks,'INPUTS/20230901_ats_oh.xls')
  df_onh= import_ats_oh (ats_oh_f)
  
  print ('...ATS bring-forward spreadsheet')
  ats_bf_f = os.path.join(wks,'INPUTS/20230901_ats_bf.xls')
  df_bfw= import_ats_bf (ats_bf_f)
  
  print('...ats report: processing time')
  ats_pt_f = os.path.join(wks,'INPUTS/20230901_ats_pt.xls')
  ...
  add_readme_page(readme_xlsx, outfile_main_rpt)
