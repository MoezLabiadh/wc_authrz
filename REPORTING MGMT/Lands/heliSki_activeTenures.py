...
def load_sql():
    ...
    sql['hsk'] = """
                SELECT
                      ...
                      ROUND(SUM(IP.AREA_HA_NUM) OVER (PARTITION BY DS.FILE_CHR),2) AS TOTAL_AREA_HA,
                      DT.LOCATION_DSC,
                      --IP.LEGAL_DSC,
                ...
                WHERE TT.STATUS_NME = 'DISPOSITION IN GOOD STANDING'
                  AND SP.SUBPURPOSE_NME = 'HELI SKI'
                
                ORDER BY OU.UNIT_NAME DESC
                """
...
def main():
    ...
    sql = load_sql()
    df = pd.read_sql(sql['hsk'],connection)
    
    df.drop_duplicates(subset=['FILE_NBR'],inplace=True)
    ...
    print ('Export report')
    create_report ([df], ['Heli Ski Active Tenures'],'heliSki_activeTenures_asof20230516')
