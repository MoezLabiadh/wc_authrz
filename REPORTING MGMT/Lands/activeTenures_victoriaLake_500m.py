...
def load_sql():
    ...
    sql['vic'] = """
        ...
        SELECT
              ...
              --OU.UNIT_NAME,
              --IP.LEGAL_DSC,
              ROUND(SDO_GEOM.SDO_DISTANCE(SH.SHAPE, LK.GEOMETRY, 0.005),2) DISTANCE_FROM_LAKE_METERS,
        ...
        FROM WHSE_TANTALIS.TA_DISPOSITION_TRANSACTIONS DT 
          ...
          JOIN WHSE_BASEMAPPING.FWA_LAKES_POLY LK
            ON SDO_WITHIN_DISTANCE (SH.SHAPE, LK.GEOMETRY, 'distance=500 unit=m') = 'TRUE'
                AND LK.GNIS_ID_1 = 30139
            ) TN
...
def main():
    ...
    sql = load_sql()
    df = pd.read_sql(sql['vic'],connection)
    ...
    print ('Export report')
    wks= r'\\spatialfiles.bcgov\Work\lwbc\visr\Workarea\moez_labiadh\WORKSPACE\2023110_activeTenures_victoraLake'
    generate_report (wks, [df], ['active tenures'],'victoriaLake_activeTenures_500m_asof20231110')
