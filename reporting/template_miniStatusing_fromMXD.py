...
def overlay_analysis (mxd,dframe,rules_dict,bcgw_conn_path,df_tit):
    ...
    for lyr in layersList[1:]:
        for k, v in rules_dict.items():
            if lyr.name == k:
                ...
                if lyr.name == 'FN Treaty Side Agreements':
                    where_clause = """ STATUS = 'ACTIVE' """

                elif lyr.name == 'Private Land':
                    where_clause = """ OWNER_TYPE = 'PRIVATE' """
                ...
                with arcpy.da.UpdateCursor('in_memory\_intersect', [area_field, 'SHAPE@AREA']) as cursor:
                    for row in cursor:
                        row[0] = round (float(row[1]/10000),4)
                ...
                in_fields = v
                cursor = arcpy.da.SearchCursor('in_memory\_intersect',in_fields)
                ...
                if k == 'FN PIP Consultation Areas':
                    df = df.groupby(['Name','CNSLTN_AREA_NAME','CONTACT_ORGANIZATION_NAME'], as_index=False)[area_field]\
                           .agg('sum')

                elif k == 'Crown Land':
                    df = pd.merge (df,df_tit, how = 'left', left_on='DISPOSITION_TRANSACTION_SID', right_on ='DTID')
                    df.drop('DTID', axis=1, inplace=True)
                    df.sort_values(by=['Name', 'CROWN_LANDS_FILE'], inplace = True)
                ...
                if df.shape [0] < 1:
                    df = df.append({'Name' : 'NO OVERLAPS FOUND!'}, ignore_index=True)
                else:
                    pass
...
def main():
    ...
    workspace = r'\\spatialfiles.bcgov\Work\lwbc\visr\Workarea\moez_labiadh\WORKSPACE\20220328_statusing_110_111_1415209'
    print ('Getting Statusing Rules\n')
    rules_dict = get_stat_rules(workspace)

    print ('Getting Tenure info \n')
    titan_report = os.path.join(workspace, 'TITAN_RPT012.xlsx')
    df_tit = get_TITAN (titan_report)
    ...
    mxd = arcpy.mapping.MapDocument(os.path.join(workspace,'proj_statusing_1415209.mxd'))
    dframe = arcpy.mapping.ListDataFrames(mxd, "Statusing_layers")[0]
    df_list, sheet_list = overlay_analysis (mxd,dframe,rules_dict,bcgw_conn_path, df_tit)

    print ('\n Creating Report')
    filename = '20220328_statusing_1415209'
